...
y_hat = bm3d_thr         (z, hadper_trans_single_den, Nstep, N1, N2, lambda_thr2D*sqrt(2*log(N1*N1)), lambda_thr3D*sqrt(2*log(N1*N1)), tau_match*N1, (Ns-1)/2, sigma, thrToIncStep, single(Tfor), single(Tinv)', inverse_hadper_trans_single_den, single(tMask), Wwin2D, smallLN, stepFS );
